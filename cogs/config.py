...
class Config(commands.Cog):
    ...
    async def setup_channel(self, interaction: discord.Interaction, channel: str):
        # When a user types #channel in a slash command Discord formats it as <#id>
        # We also accept a bare channel ID for convenience
        clean = channel.strip("<#>").strip()
        if not clean.isdigit():
            await interaction.response.send_message(
                "⚠️ Please mention the channel directly, e.g. type `#general` in the field.",
                ephemeral=True,
            )
            return
        ...
        if not isinstance(resolved, discord.TextChannel):
            await interaction.response.send_message(
                "⚠️ That doesn't look like a text channel. Try mentioning it with `#`.",
                ephemeral=True,
            )
            return
